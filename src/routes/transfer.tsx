...
function TransferPage() {
  ...
  const validate = () => {
    ...
    if (!isValidMpayForTx(mpayCode)) {
      setMpayError("Invalid MPAY ID CODE");
      return false;
    }
    const amt = Number(amount);
    if (!amt || amt <= 0) { setError("Enter a valid amount"); return false; }
    if (amt > balance) { setError("Insufficient balance"); return false; }
    return true;
  };

  const proceed = () => { if (validate()) setStep("confirm"); };
    ...
    <PhoneFrame>
      <div className="flex-1 flex flex-col" style={{ background: PRIMARY, color: "#FFFFFF" }}>
        <div className="px-6 pt-10 pb-5 relative overflow-hidden">
          ...
          <div className="mt-3 rounded-2xl bg-white/15 backdrop-blur p-3 flex items-center justify-between border border-white/20">
            <div>
                    ...
                    <div className="mt-2 max-h-48 overflow-y-auto rounded-xl bg-white text-foreground">
                      ...
                      {filteredBanks.map((b) => (
                        <button key={b} type="button"
                          ...
                          className="w-full text-left px-4 py-2.5 text-sm hover:bg-brand-soft border-b border-border last:border-0">
                          {b}
                        </button>
                      ))}
                    </div>
